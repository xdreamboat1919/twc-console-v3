...
function dpReadForm(){
  var record={};
  Object.keys(DP_FIELDS).forEach((key)=> {record[key]=$(DP_FIELDS[key]).value.trim()});
  DP_NUMERIC.forEach((key)=> {record[key]=opsNum(record[key])});
  return record;
}
function dpWriteForm(record){
  Object.keys(DP_FIELDS).forEach((key)=> {
...
function dpSignal(record){
  var metric=dpMetrics(record),gap=Math.abs(metric.orderGap==null?0:metric.orderGap),revenueGap=Math.abs(metric.revenueGap==null?0:metric.revenueGap),rule=typeof wsRuleValue==='function'?((key,fallback)=> wsRuleValue(key,record,fallback)):((key,fallback)=> fallback);
...
function dpUpdateDateFilter(){
  ...
  var dates=[].concat(dailyRecords.map((record)=> record.date).filter(Boolean)).filter((date,index,list)=> list.indexOf(date)===index).sort().reverse();
  filter.innerHTML='<option value="latest">Latest date</option><option value="">All dates</option>'+dates.map((date)=> '<option value="'+opsEsc(date)+'">'+opsEsc(date)+'</option>').join('');
...
function renderDaily(){
  ...
  var scopedRecords=dailyRecords.filter((record)=> (!selectedDate||record.date===selectedDate)&&(!platform||record.platform===platform));
  var totals=scopedRecords.reduce((sum,record)=> {
  ...
  var records=scopedRecords.slice().sort((a,b)=> b.date.localeCompare(a.date)).filter((record)=> {
  ...
  $('dp-rows').innerHTML=records.length?records.map((record)=> {
...
function dpSave(){
  ...
  var duplicate=dailyRecords.find((item)=> item.id!==dpEditing&&typeof workspaceImportNameKey==='function'&&workspaceImportNameKey(item)===workspaceImportNameKey(record));
  ...
  if(dpEditing){
    var current=dailyRecords.find((item)=> item.id===dpEditing);
...
function dpEdit(id){
  var record=dailyRecords.find((item)=> item.id===id);if(!record)return;
...
function dpCreateAction(id){
  var record=dailyRecords.find((item)=> item.id===id);if(!record)return;
  var existing=optimizationActions.find((item)=> item.sourceId===id&&item.status!=='Resolved');
...
function dpImportCsv(file){
  ...
  reader.onload=()=> {
    try{
      ...
      var headers=rows.shift().map((header)=> header.trim().toLowerCase().replace(/[^a-z0-9]+/g,'_').replace(/^_|_$/g,''));
      ['date','account','campaign','spend'].forEach((required)=> {if(headers.indexOf(required)<0)throw new Error('Missing '+required)});
      ...
      rows.forEach((values)=> {
        ...
        headers.forEach((header,column)=> {var key=map[header]||header;if(Object.prototype.hasOwnProperty.call(DP_FIELDS,key))record[key]=(values[column]||'').trim()});
        DP_NUMERIC.forEach((key)=> {record[key]=opsNum(record[key])});
...
function dpExportCsv(){
  ...
  var rows=[DP_CSV_COLUMNS.join(',')].concat(dailyRecords.map((record)=> keys.map((key)=> dpCsvValue(record[key])).join(',')));
...
function aqReadForm(){var item={};Object.keys(AQ_FIELDS).forEach((key)=> {item[key]=$(AQ_FIELDS[key]).value.trim()});return item}
function aqWriteForm(item){Object.keys(AQ_FIELDS).forEach((key)=> {$(AQ_FIELDS[key]).value=item&&item[key]!=null?item[key]:''})}
...
function aqSave(){
  ...
  if(aqEditing){var current=optimizationActions.find((entry)=> entry.id===aqEditing);if(current){Object.assign(current,item);if(typeof workspaceAudit==='function')workspaceAudit('Decision','Optimization action updated',item.problem,current.id)}toast('Queue item updated')}
...
function aqEdit(id){var item=optimizationActions.find((entry)=> entry.id===id);if(!item)return;aqEditing=id;aqWriteForm(item);$('aq-save').textContent='Update queue item';$('aq-cancel').hidden=false;window.scrollTo(0,0)}
function aqLogChange(id){
  var item=optimizationActions.find((entry)=> entry.id===id);if(!item)return;
  clResetForm();clSourceAction=id;
  var source=dailyRecords.find((record)=> record.id===item.sourceId);
...
function renderActions(){
  var active=optimizationActions.filter((item)=> item.status!=='Resolved'),overdue=active.filter((item)=> opsIsOverdue(item.due,item.status));
  $('aq-open').textContent=active.filter((item)=> item.status==='Open'||item.status==='Actioned').length;
  $('aq-critical').textContent=active.filter((item)=> item.priority==='Critical').length;
  $('aq-overdue').textContent=overdue.length;
  $('aq-monitoring').textContent=optimizationActions.filter((item)=> item.status==='Monitoring').length;
  $('aq-resolved').textContent=optimizationActions.filter((item)=> item.status==='Resolved').length;
  opsSetStatBox('aq-critical-box',active.some((item)=> item.priority==='Critical')?'lo':'');
  ...
  var items=optimizationActions.slice().sort((a,b)=> (order[a.priority]-order[b.priority])||(a.due||'9999').localeCompare(b.due||'9999')).filter((item)=> {
  ...
  $('aq-rows').innerHTML=items.length?items.map((item)=> {
...
function clReadForm(){var item={};Object.keys(CL_FIELDS).forEach((key)=> {item[key]=$(CL_FIELDS[key]).value.trim()});item.cooldown=opsNum(item.cooldown);return item}
function clWriteForm(item){Object.keys(CL_FIELDS).forEach((key)=> {$(CL_FIELDS[key]).value=item&&item[key]!=null?item[key]:''})}
...
function clCheckOverlap(){
  ...
  var overlap=scope&&changeRecords.find((item)=> item.id!==clEditing&&item.scope.toLowerCase()===scope&&clDisplayStatus(item)==='Cooldown'&&(!item.recheck||item.recheck>=opsToday()));
...
function clSave(){
  ...
  if(clEditing){var current=changeRecords.find((entry)=> entry.id===clEditing);if(current){Object.assign(current,item);if(typeof workspaceAudit==='function')workspaceAudit('Decision','Controlled change updated',item.scope+' · '+item.reason,current.id)}toast('Change updated')}
  else{item.id=opsId('change');item.created=new Date().toISOString();item.sourceActionId=clSourceAction;changeRecords.push(item);if(clSourceAction){var action=optimizationActions.find((entry)=> entry.id===clSourceAction);if(action&&action.status==='Open')action.status='Actioned'}if(typeof workspaceAudit==='function')workspaceAudit('Decision','Controlled change logged',item.scope+' · '+item.reason,item.id);toast('Change logged')}
  clResetForm();renderChanges();renderActions();
}
function clEdit(id){var item=changeRecords.find((entry)=> entry.id===id);if(!item)return;clEditing=id;clSourceAction=item.sourceActionId||'';clWriteForm(item);$('cl-save').textContent='Update change';$('cl-cancel').hidden=false;clCheckOverlap();window.scrollTo(0,0)}
function renderChanges(){
  var derived=changeRecords.map((item)=> ({item:item,status:clDisplayStatus(item)}));
  $('cl-cooldowns').textContent=derived.filter((entry)=> entry.status==='Cooldown').length;
  $('cl-ready').textContent=derived.filter((entry)=> entry.status==='Ready to review').length;
  $('cl-success').textContent=derived.filter((entry)=> entry.status==='Successful').length;
  $('cl-inconclusive').textContent=derived.filter((entry)=> entry.status==='Inconclusive').length;
  opsSetStatBox('cl-ready-box',derived.some((entry)=> entry.status==='Ready to review')?'md':'');
  var query=$('cl-search').value.toLowerCase().trim(),filter=$('cl-filter-status').value;
  var items=changeRecords.slice().sort((a,b)=> b.date.localeCompare(a.date)).filter((item)=> {var status=clDisplayStatus(item),haystack=(item.scope+' '+item.reason+' '+item.expected+' '+item.result).toLowerCase();return (!query||haystack.indexOf(query)>-1)&&(!filter||status===filter)});
  $('cl-count').textContent=items.length+' of '+changeRecords.length+' changes';
  $('cl-rows').innerHTML=items.length?items.map((item)=> {var status=clDisplayStatus(item);return '<tr>'+ 
...
function xtReadForm(){var item={};Object.keys(XT_FIELDS).forEach((key)=> {item[key]=$(XT_FIELDS[key]).value.trim()});return item}
function xtWriteForm(item){Object.keys(XT_FIELDS).forEach((key)=> {$(XT_FIELDS[key]).value=item&&item[key]!=null?item[key]:''})}
...
function xtSave(){
  ...
  if(xtEditing){var current=experimentRecords.find((entry)=> entry.id===xtEditing);if(current){Object.assign(current,item);if(typeof workspaceAudit==='function')workspaceAudit('Decision','Experiment updated',item.name,current.id)}toast('Experiment updated')}
...
function xtEdit(id){var item=experimentRecords.find((entry)=> entry.id===id);if(!item)return;xtEditing=id;xtWriteForm(item);$('xt-save').textContent='Update experiment';$('xt-cancel').hidden=false;window.scrollTo(0,0)}
function xtCreateAction(id){
  var item=experimentRecords.find((entry)=> entry.id===id);if(!item)return;
  var sourceId='experiment:'+id,existing=optimizationActions.find((action)=> action.sourceId===sourceId&&action.status!=='Resolved');
...
function renderExperiments(){
  $('xt-planned').textContent=experimentRecords.filter((item)=> item.status==='Planned').length;
  $('xt-running').textContent=experimentRecords.filter((item)=> item.status==='Running').length;
  var due=experimentRecords.filter((item)=> item.status!=='Complete'&&item.end&&item.end<=opsToday()).length;
  $('xt-due').textContent=due;$('xt-complete').textContent=experimentRecords.filter((item)=> item.status==='Complete').length;opsSetStatBox('xt-due-box',due?'md':'');
  var query=$('xt-search').value.toLowerCase().trim(),filter=$('xt-filter-status').value;
  var items=experimentRecords.slice().sort((a,b)=> (a.end||'9999').localeCompare(b.end||'9999')).filter((item)=> {var haystack=(item.name+' '+item.product+' '+item.owner+' '+item.hypothesis).toLowerCase();return (!query||haystack.indexOf(query)>-1)&&(!filter||item.status===filter)});
  $('xt-count').textContent=items.length+' of '+experimentRecords.length+' experiments';
  $('xt-rows').innerHTML=items.length?items.map((item)=> '<tr>'+ 
...
function renderOpsHome(){
  ...
  $('home-alerts').textContent=dailyRecords.filter((record)=> dpSignal(record).level!=='healthy').length;
  $('home-actions').textContent=optimizationActions.filter((item)=> item.status!=='Resolved').length;
  $('home-overdue').textContent=optimizationActions.filter((item)=> opsIsOverdue(item.due,item.status)).length;
  $('home-rechecks').textContent=changeRecords.filter((item)=> clDisplayStatus(item)==='Ready to review').length;
  $('home-tests').textContent=experimentRecords.filter((item)=> item.status==='Running').length;
...
function opsReportSummary(){
  var open=optimizationActions.filter((item)=> item.status!=='Resolved'),ready=changeRecords.filter((item)=> clDisplayStatus(item)==='Ready to review'),running=experimentRecords.filter((item)=> item.status==='Running'),completed=experimentRecords.filter((item)=> item.status==='Complete');
  var lines='12  OPERATIONAL DECISIONS\n'+
    '    Open actions          '+open.length+'\n'+
    '    Critical actions      '+open.filter((item)=> item.priority==='Critical').length+'\n'+
    '    Overdue actions       '+open.filter((item)=> opsIsOverdue(item.due,item.status)).length+'\n'+
    '    Changes due to read   '+ready.length+'\n';
  open.slice(0,3).forEach((item)=> {lines+='    · '+item.priority+' — '+item.problem+'\n'});
  lines+='\n13  EXPERIMENT PIPELINE\n'+
    '    Running               '+running.length+'\n'+
    '    Completed             '+completed.length+'\n';
  running.slice(0,3).forEach((item)=> {lines+='    · '+item.name+' — read '+item.end+'\n'});
  return lines+'\n';
}
...
function opsAggregatePeriod(startExclusive,endInclusive){
  return dailyRecords.filter((record)=> record.date>startExclusive&&record.date<=endInclusive).reduce((sum,record)=> {
...
$('dp-template').addEventListener('click',()=> {
...
$('dp-import').addEventListener('click',()=> {$('dp-file').click()});
$('dp-file').addEventListener('change',(event)=> {var file=event.target.files[0];if(file)dpImportCsv(file);event.target.value=''});
$('dp-rows').addEventListener('click',(event)=> {
...
$('aq-rows').addEventListener('click',(event)=> {
  var button=event.target.closest('button');if(!button)return;
