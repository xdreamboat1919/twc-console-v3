...
function calcBrandInc() {
  var sp = +$('gb-sp').value || 0,
    cv = +$('gb-cv').value || 0,
    aov = +$('gb-aov').value || 0,
    org = +$('gb-org').value || 0,
    can = (+$('gb-can').value || 0) / 100,
    comp = $('gb-comp').value === '1';
  var eff = comp ? Math.max(0, can - 0.25) : can;
  var inc = cv * (1 - eff);
  var cpa = cv ? sp / cv : 0,
    tcpa = inc ? sp / inc : 0;
  var roas = sp ? (cv * aov) / sp : 0,
    iroas = sp ? (inc * aov) / sp : 0;
  ...
  if (comp && eff < 0.5) {
    cls = 'is-warn';
    label = 'Defensive spend, partly justified';
    val = 'DEFEND';
    body =
      'Competitors are bidding on your brand, which is the one condition that makes branded search genuinely incremental. Someone searching your name who sees a competitor first is a customer you can lose. Effective cannibalisation drops to ' +
      (eff * 100).toFixed(0) +
      ' percent, giving a true CPA of $' +
      tcpa.toFixed(0) +
      ' against a reported $' +
      cpa.toFixed(0) +
    ...
    cls = 'is-fail';
    label = 'Mostly buying traffic you already had';
    val = 'CANNIBAL';
    body =
      'At ' +
      (eff * 100).toFixed(0) +
      ' percent cannibalisation, roughly ' +
      Math.round(cv * eff) +
      ' of ' +
      cv +
      ' monthly conversions would have arrived through organic anyway. True CPA is $' +
      tcpa.toFixed(0) +
      ' against a reported $' +
      cpa.toFixed(0) +
      ', and roughly ' +
      money(annual) +
      ' a year is being spent to appear above your own organic listing. Reported ROAS of ' +
      roas.toFixed(2) +
      ' is the number that makes this look like the best channel in the account, and incremental ROAS of ' +
      iroas.toFixed(2) +
      ' is closer to what it earns. Run the pause test before reallocating, because the assumption is doing all the work here.';
  } else {
    cls = 'is-warn';
    label = 'Partly incremental';
    val = 'MIXED';
    body =
      'At ' +
      (eff * 100).toFixed(0) +
      ' percent cannibalisation, true CPA of $' +
      tcpa.toFixed(0) +
      ' against a reported $' +
      cpa.toFixed(0) +
  ...
  r.classList.add(cls);
  $('gb-label').textContent = label;
  $('gb-val').textContent = val;
  $('gb-body').textContent = body;
  $('gb-test').textContent =
...
['gb-sp', 'gb-cv', 'gb-aov', 'gb-org', 'gb-can', 'gb-comp'].forEach((id) => {
  $(id).addEventListener('input', calcBrandInc);
});

var GSPEC = {
  brand: {
    n: 'Branded search',
    bid: 'Manual CPC or Target impression share',
    match: 'Exact and phrase only',
  ...
  nonbrand: {
    n: 'Non-brand search',
    bid: 'Maximise conversions, then Target CPA',
    match: 'Phrase and exact, broad only with strong negatives',
  ...
  shop: {
    n: 'Standard Shopping',
    bid: 'Target ROAS once data allows',
    match: 'Feed-driven, no keywords',
  ...
  pmax: {
    n: 'Performance Max',
    bid: 'Target ROAS with a floor',
    match: 'Automated across all inventory',
  ...
  dgen: {
    n: 'Demand Gen',
    bid: 'Maximise conversions',
    match: 'Audience and creative driven',
  ...
  yt: {
    n: 'YouTube',
    bid: 'Target CPA or CPM by objective',
    match: 'Audience and placement driven',
...
function renderGStruct() {
  ...
  $('gs-spec').innerHTML = [
    ...
    ['Matching', G.match],
    [
      'Weekly conversions at this budget',
      conv.toFixed(1) + (conv < 15 ? '  ·  below the 30-in-30 threshold for smart bidding' : ''),
    ],
  ]
    .map(
      (r) =>
        '<tr><td style="width:200px;color:var(--ink-3);font-size:12.5px;vertical-align:top">' +
        r[0] +
        '</td><td>' +
        r[1] +
        '</td></tr>',
  ...
  if (conv < 15 && (k === 'nonbrand' || k === 'pmax' || k === 'shop')) {
    ...
    n.textContent =
      ...
      ' daily against a $' +
      cpa +
      ' target, this campaign produces roughly ' +
      conv.toFixed(1) +
      ' conversions a week. Smart bidding needs around thirty conversions in thirty days before Target CPA or Target ROAS behaves, and below that it will underdeliver rather than optimise. Start on maximise conversions, or consolidate into fewer campaigns until the volume supports the bidding strategy. ' +
      G.note;
  } else {
    n.className = 'note';
...
['gs-type', 'gs-lane', 'gs-day', 'gs-cpa'].forEach((id) => {
  $(id).addEventListener('input', renderGStruct);
});

var GWCAT = [
  ['Brand terms', 1, 'Review the incrementality question above, not the CPA'],
...
function calcGWaste() {
  ...
  GWCAT.forEach((c) => {
    var i = c[1];
    var sp = +$('gw-s' + i).value || 0,
      cv = +$('gw-c' + i).value || 0;
    var k = cv ? sp / cv : 0;
    $('gw-k' + i).textContent = cv ? '$' + k.toFixed(0) : 'no conv';
    $('gw-a' + i).innerHTML =
      '<span style="font-size:12.5px;color:var(--ink-3)">' + c[2] + '</span>';
    tot += sp;
    if (i >= 4) waste += sp;
    if (i === 1) brand = sp;
    rows.push({ n: c[0], sp: sp, cv: cv, k: k });
  });
  var pct = tot ? (waste / tot) * 100 : 0,
    bpct = tot ? (brand / tot) * 100 : 0;
  ...
  if (pct >= 18) {
    msg =
      'Informational and irrelevant terms carry ' +
      pct.toFixed(0) +
      ' percent of search spend, or ' +
      money(waste * 12) +
    ...
    msg =
      'Waste at ' +
      pct.toFixed(0) +
      ' percent is within a normal range for an account with broad or phrase match running, and it is still ' +
      money(waste * 12) +
    ...
    msg =
      'Waste at ' +
      pct.toFixed(0) +
      ' percent is tight, which usually means match types are conservative. Worth checking whether that is also limiting reach on the exact-intent terms that convert. ';
  }
  if (bpct >= 45) {
    msg +=
      'Separately, brand carries ' +
      bpct.toFixed(0) +
...
GWCAT.forEach((c) => {
  ['s', 'c'].forEach((f) => {
    var id = 'gw-' + f + c[1];
    if ($(id)) $(id).addEventListener('input', calcGWaste);
  });
});
...
function gvApply() {
  $('gt-canvas').style.transform = 'translate(' + gv.x + 'px,' + gv.y + 'px) scale(' + gv.s + ')';
  $('gt-zoom').textContent = Math.round(gv.s * 100) + '%';
}
function gvPrep() {
  var g = $('gt-canvas').querySelector('svg');
  if (!g) return;
...
function gvFit() {
  ...
  gv.x = (st.clientWidth - w * gv.s) / 2;
  gv.y = (st.clientHeight - h * gv.s) / 2;
  gvApply();
}
function gvZoom(f, cx, cy) {
  ...
  gv.x = cx - (cx - gv.x) * (ns / gv.s);
  gv.y = cy - (cy - gv.y) * (ns / gv.s);
  gv.s = ns;
  gvApply();
}
(() => {
  ...
  st.addEventListener(
    ...
    (e) => {
      e.preventDefault();
      gvZoom(e.deltaY < 0 ? 1.12 : 1 / 1.12, e.clientX, e.clientY);
    },
    { passive: false },
  );
  ...
  st.addEventListener('pointermove', (e) => {
    if (!gv.drag) return;
    gv.x += e.clientX - gv.px;
    gv.y += e.clientY - gv.py;
    gv.px = e.clientX;
    gv.py = e.clientY;
    gvApply();
  });
  ...
  st.addEventListener('dblclick', (e) => {
    if (!e.target.closest('.mmctl')) gvZoom(1.5, e.clientX, e.clientY);
  });
})();
$('gz-in').addEventListener('click', () => {
  gvZoom(1.25);
});
$('gz-out').addEventListener('click', () => {
  gvZoom(1 / 1.25);
});
$('gz-fit').addEventListener('click', gvFit);
$('gz-pres').addEventListener('click', () => {
  var st = $('gt-stage'),
    on = st.classList.toggle('present');
  document.body.classList.toggle('presenting', on);
  $('gz-pres').textContent = on ? 'Exit' : 'Present';
  setTimeout(gvFit, 60);
});
$('gz-svg').addEventListener('click', () => {
  var g = $('gt-canvas').querySelector('svg');
  if (!g) return;
  ...
  dl(
    ...
    'google-' + $('gs-type').value + '.svg',
  );
});
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && $('gt-stage').classList.contains('present')) {
    $('gz-pres').click();
  }
});
...
function googleMermaid() {
  ...
  if (k === 'brand') {
    ...
    m +=
      '  classDef camp fill:#14213d,stroke:#14213d,color:#ffffff\n  classDef bad fill:#fbeaea,stroke:#96302f,color:#96302f\n';
    m +=
    ...
    m +=
      '  C["NONBRAND_' + L + '<br/>' + money(day) + ' / day<br/>Max conversions until 30 in 30"]\n';
    m +=
      '  C --> A1["AG · Theme 1<br/>tight keyword set"]\n  C --> A2["AG · Theme 2"]\n  C --> A3["AG · Theme 3"]\n';
    m +=
      '  A1 --> K1["Phrase + exact"]\n  K1 --> R1["2 RSAs<br/>different angles<br/>one pinned, one not"]\n';
    m +=
      '  N(["Negatives, built BEFORE launch<br/>free · cheap · diy · how to make<br/>side effects · recall · lawsuit"])\n  N -.-> C\n';
    ...
    m +=
      '  C --> AG1["Asset group · PREPAREDNESS<br/>5 headlines · 5 desc<br/>3 image ratios · 1 video"]\n';
    ...
    m += '  BX2{"Off?"}\n  C -.-> BX2\n';
    m +=
      '  BX2 -->|off| CANN["Absorbs branded search<br/>reports it as new performance<br/>hollows out the brand campaign"]\n';
    m +=
      '  BLIND(["Reporting limits:<br/>placement cost · search-term detail<br/>asset conversions are non-additive"])\n';
    m += '  C -.-> BLIND\n';
    m +=
      '  classDef camp fill:#14213d,stroke:#14213d,color:#ffffff\n  classDef bad fill:#fbeaea,stroke:#96302f,color:#96302f\n';
    m += '  classDef meta fill:#f2f5f9,stroke:#c2cbd8,color:#6b7889\n';
    ...
    m +=
      '  C --> A1["AG · In-stream skippable<br/>reach and consideration"]\n  C --> A2["AG · In-feed<br/>intent"]\n';
    m +=
      '  A1 --> V["Video<br/>first 5 seconds carry the weight<br/>product visible before the skip"]\n';
    m += '  META["Meta hook-led video"] -.->|"3s rate predicts 5s rate"| V\n';
    m += '  C --> M{"Reported conversions"}\n';
    m +=
      '  M -->|"heavily view-through"| WARN["Weakest signal available<br/>judge on total business volume<br/>not on platform numbers"]\n';
    m +=
      '  classDef camp fill:#14213d,stroke:#14213d,color:#ffffff\n  classDef bad fill:#fbeaea,stroke:#96302f,color:#96302f\n';
  ...
  return {
    ...
    cap: cap,
    title: GSPEC[k].n,
    sub: L + ' · ' + money(day) + ' daily · $' + cpa + ' target',
  };
}
...
/* ================= GOOGLE SCALING ================= */
function calcGScale() {
  var is = +$('gi-is').value || 0,
    bud = +$('gi-bud').value || 0,
    rank = +$('gi-rank').value || 0,
    sp = +$('gi-sp').value || 0,
    cpa = +$('gi-cpa').value || 1,
    mx = +$('gi-max').value || 1;
  var head = bud + rank;
  ...
  if (head < 10) {
    cls = 'is-warn';
    label = 'Near the ceiling on this keyword set';
    val = 'EXPAND';
    body =
      'Impression share of ' +
      is +
    ...
    cls = 'is-pass';
    label = 'Budget limited, straightforward';
    val = 'BUDGET';
    body =
      'You are losing ' +
      bud +
      ' percent of impression share to budget against ' +
      rank +
      ' percent to rank, which means the auction is winnable and you simply are not showing up for it. Raising budget could take impression share from ' +
      is +
      ' to roughly ' +
      atFullBudget.toFixed(0) +
      ' percent, around ' +
      mult.toFixed(2) +
      ' times current volume at roughly ' +
      money(spendNeeded) +
  ...
  r.classList.add(cls);
  $('gi-label').textContent = label;
  $('gi-val').textContent = val;
  $('gi-body').textContent = body;
  var levers = [
    [
      'Impression share, budget',
      'Direct and measured',
      'Raise budget where lost IS to budget exceeds lost IS to rank. The only lever on either platform where the ceiling is reported rather than inferred.',
      bud > rank ? 'f-ok' : 'f-low',
    ],
    [
      'Quality Score and rank',
      'Cheaper than bidding',
      'Ad relevance, expected CTR and landing page experience. Improves position without raising cost per click, and the landing page work improves conversion rate at the same time.',
      rank > bud ? 'f-ok' : 'f-low',
    ],
    [
      'Keyword expansion',
      'From the search terms report',
      'Converting terms you did not choose become new exact keywords. This is the scaling input most accounts already have and never harvest.',
      head < 15 ? 'f-ok' : 'f-low',
    ],
    [
      'Match type loosening',
      'Only with mature negatives',
      'Phrase to broad opens volume and opens waste in the same move. Do not do this until the negative list has been through eight weeks of weekly review.',
      'f-low',
    ],
    [
      'Campaign type expansion',
      'Search to Shopping to PMax to Demand Gen',
      'Each type reaches demand the others cannot. Demand Gen is where Meta creative transfers, which makes it the cheapest expansion if the creative already exists.',
      head < 12 ? 'f-ok' : 'f-low',
    ],
    [
      'Bid strategy loosening',
      'Last, and measured',
      'Raising Target CPA finds more volume at a worse rate by definition. Use it to discover the ceiling, not to hold a position past it.',
      'f-low',
    ],
  ];
  $('gi-levers').innerHTML = levers
    .map(
      (l, i) =>
        '<div class="flag ' +
        l[3] +
...
['gi-is', 'gi-bud', 'gi-rank', 'gi-sp', 'gi-cpa', 'gi-max'].forEach((id) => {
  $(id).addEventListener('input', calcGScale);
});
