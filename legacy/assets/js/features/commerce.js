...
function calcShopRec(){
  var so=+$('sh-so').value||0, sr=+$('sh-sr').value||0, tg=+$('sh-tag').value||0, mt=+$('sh-meta').value||0;
  var aov=so?sr/so:0, over=so?((mt-so)/so*100):0, untag=so-tg, orgp=so?((so-tg)/so*100):0;
  ...
  var n=$('sh-rnote');
  if(a<=15){n.className='note good';n.textContent='Meta over-reports by '+over.toFixed(0)+' percent, which is inside normal attribution difference. Meta answers how many purchases were influenced within its window. Shopify answers how many transactions happened. Both are correct and they will not agree.';}
  else if(over>0){n.className='note bad';n.textContent='Meta reports '+over.toFixed(0)+' percent more purchases than Shopify recorded orders. Beyond attribution difference, the usual causes in this account are subscription renewals firing the Purchase event, a thank-you page reload double counting, or Rx orders firing at payment rather than at fulfilment. Work those three before adjusting any campaign.';}
  else{n.className='note warn';n.textContent='Shopify shows more orders than Meta reports, which usually means genuine organic or unattributed demand rather than a fault. '+untag.toLocaleString()+' orders carry no paid media tag, or '+orgp.toFixed(0)+' percent of the total. Confirm the tagging is complete before treating that as organic, because untagged paid orders look identical to organic ones.';}
}
['sh-so','sh-sr','sh-tag','sh-meta'].forEach(function(id){$(id).addEventListener('input',calcShopRec)});
