/* ================= PIXEL AND CAPI ================= */
var PXA=[
 ['px1','Pixel installed and firing on every commerce page',10,'Verify with Pixel Helper on the live storefront, not the homepage alone.'],
 ['px2','CAPI live and receiving server events',10,'Server events visible in Events Manager alongside browser events.'],
 ['px3','Deduplication working on Purchase',10,'Matched event_id, confirmed in Test Events rather than assumed.'],
 ['px4','Deduplication working on every other event',8,'AddToCart and InitiateCheckout are the ones commonly missed. Purchase looks clean while every mid-funnel rate is inflated.'],
 ['px5','event_id is the order ID, on both sides',8,'A timestamp or random ID will never match, and it also fails to catch a thank-you page reload.'],
 ['px6','Purchase value and currency accurate',10,'Compare five real orders against the back end. A currency mismatch reads as a revenue gap with matching order counts.'],
 ['px7','Subscription renewals fire a distinct event',8,'If renewals fire Purchase, Meta optimises toward a blend of new and existing customers and reported ROAS is flattered by revenue the ads did not generate.'],
 ['px8','Rx conversion fires on fulfilment, not payment',10,'Payment precedes clinical approval on this funnel. Firing at payment trains delivery toward orders that may never ship.'],
 ['px9','Domain verified in Business Manager',7,'Required for Aggregated Event Measurement and for controlling who can configure events on your domain.'],
 ['px10','Aggregated Event Measurement configured, Purchase first',6,'Eight slots, ordered by priority. Wrong order means the wrong event survives on iOS.'],
 ['px11','Advanced matching enabled on commerce surfaces',6,'Improves match rates materially and belongs only on Class A.'],
 ['px12','Clinical surfaces excluded from browser pixel entirely',10,'No pixel, no SDK on intake, questionnaire or records pages. Server-side from the CRM at a de-identified milestone instead.'],
 ['px13','Verified end to end in Test Events before launch',5,'Not after. A fault found in week three has already corrupted three weeks of decisions.'],
 ['px14','Diagnostics reviewed weekly',4,'Dedup and parameter coverage break silently on theme updates, app updates and checkout changes.']
];
var pxOn={px1:1,px2:1,px3:1,px9:1,px11:1};
function renderPixel(){
  $('px-list').innerHTML=PXA.map(function(a){
    var on=pxOn[a[0]];
    return '<div class="chk'+(on?' ok':'')+'"><input type="checkbox" data-px="'+a[0]+'"'+(on?' checked':'')+'>'+
      '<div class="t">'+a[1]+'<small>'+a[3]+'</small></div>'+
      '<span class="b" style="background:'+(a[2]>=10?'var(--fail-bg)':a[2]>=7?'var(--warn-bg)':'var(--panel-2)')+
      ';color:'+(a[2]>=10?'var(--fail)':a[2]>=7?'var(--warn)':'var(--ink-3)')+'">'+a[2]+'</span></div>';
  }).join('');
  var total=PXA.reduce(function(x,a){return x+a[2]},0);
  var got=PXA.reduce(function(x,a){return x+(pxOn[a[0]]?a[2]:0)},0);
  var pct=Math.round(got/total*100);
  var r=$('px-read'); r.className='readout'; var cls,label,body;
  var criticalGaps=PXA.filter(function(a){return a[2]>=10&&!pxOn[a[0]]});
  if(criticalGaps.length){cls='is-fail';label='Not safe to optimise against';
    body=criticalGaps.length+' critical dimension'+(criticalGaps.length===1?' is':'s are')+' failing. At this state the conversion data is wrong in a way that will not announce itself, and every scaling decision, creative verdict and budget conversation built on it will be confidently incorrect. Fix these before changing anything in the account, because changes made now cannot be attributed to anything afterwards.';}
  else if(pct<85){cls='is-warn';label='Usable, with known gaps';
    body='No critical faults and '+(100-pct)+' percent of the weighted checks outstanding. The numbers can be acted on, and the remaining gaps will each cost something specific rather than nothing. Work them in the order below.';}
  else{cls='is-pass';label='Trustworthy';
    body='The measurement layer is sound. That means a scaling decision, a creative kill and a budget increase can all be made on the reported numbers without a reconciliation argument first, which is the entire point of the work.';}
  r.classList.add(cls);
  ...
  var gaps=PXA.filter(function(a){return !pxOn[a[0]]}).sort(function(a,b){return b[2]-a[2]});
  $('px-gaps').innerHTML=gaps.length?gaps.map(function(a,i){
    var c=a[2]>=10?'f-high':a[2]>=7?'f-med':'f-low';
    return '<div class="flag '+c+'"><b>'+(i+1)+'. '+a[1]+'</b>'+a[3]+'</div>';
  }).join(''):'<div class="flag f-ok"><b>Nothing outstanding</b>Every dimension clears. Keep the weekly Diagnostics review running, because this state does not hold on its own.</div>';
}
$('px-list').addEventListener('change',function(e){
  if(e.target.type!=='checkbox')return;
  pxOn[e.target.dataset.px]=e.target.checked?1:0; renderPixel();
});

var PXP=[
 ['em','Email, hashed',10,'The single strongest match signal.'],
 ['ph','Phone, hashed',8,'Second strongest, and frequently missing because checkout does not require it.'],
 ['ex','external_id',7,'Your own customer ID, hashed. Persists where cookies do not.'],
 ['fbc','fbc, the click ID',9,'Captured from fbclid on landing. Missing this is the most common single cause of poor matching.'],
 ['fbp','fbp, the browser ID',7,'Set by the pixel. Requires the pixel to have fired first.'],
 ['fn','First name',4,''],['ln','Last name',4,''],
 ['ct','City',3,''],['st','State',3,''],['zp','Postcode',5,''],['co','Country',4,''],
...
function renderParams(){
  $('px-params').innerHTML='<div class="tw"><table><thead><tr><th>Parameter</th><th style="text-align:right">Impact</th><th>Sending</th><th>Note</th></tr></thead><tbody>'+
   PXP.map(function(p){
     return '<tr><td style="font-family:var(--mono);font-size:12.5px">'+p[0]+'</td><td class="num">'+p[2]+'</td>'+
       '<td><input type="checkbox" data-pq="'+p[0]+'"'+(pqOn[p[0]]?' checked':'')+' style="width:16px;height:16px;accent-color:var(--act)"></td>'+
       '<td style="font-size:12.5px;color:var(--ink-3)">'+(p[3]||'')+'</td></tr>';
   }).join('')+'</tbody></table></div>';
  var total=PXP.reduce(function(x,p){return x+p[2]},0);
  var got=PXP.reduce(function(x,p){return x+(pqOn[p[0]]?p[2]:0)},0);
  var n=PXP.filter(function(p){return pqOn[p[0]]}).length;
  var pct=Math.round(got/total*100);
  var emq=pct>=75?'8 to 10':pct>=55?'6 to 8':pct>=35?'4 to 6':'below 4';
  var gaps=PXP.filter(function(p){return !pqOn[p[0]]}).sort(function(a,b){return b[2]-a[2]});
  ...
  if(gaps.length&&gaps[0][2]>=8){
    ...
    nn.textContent='The largest gap is '+gaps[0][1].toLowerCase()+', weighted '+gaps[0][2]+'. '+(gaps[0][3]||'')+
      ' Adding it is usually a back-end change rather than a checkout change, which makes it cheaper than it sounds. Coverage of '+pct+' percent puts the likely Event Match Quality band around '+emq+'.';
  } else if(pct>=70){
    nn.className='note good';
    nn.textContent='Coverage of '+pct+' percent across '+n+' parameters, which should put Event Match Quality around '+emq+'. Further gains from here are marginal, and effort is better spent on event timing and deduplication than on chasing the last few parameters.';
  } else {
    nn.className='note warn';
    nn.textContent='Coverage of '+pct+' percent, likely landing Event Match Quality around '+emq+'. The remaining gaps are lower impact individually and add up. Prioritise anything above a weight of 7 and leave the rest.';
  }
}
$('px-params').addEventListener('change',function(e){
  if(e.target.type!=='checkbox')return;
  pqOn[e.target.dataset.pq]=e.target.checked?1:0; renderParams();
});

var PXSPEC={
 commerce:{t:'CLASS A · COMMERCE · twc.health and twccanada.health\n'+
  '='.repeat(58)+'\n\n'+
 ...
 rx:{t:'PRESCRIPTION · TIMING-CRITICAL\n'+
  '='.repeat(58)+'\n\n'+
  'THE PROBLEM\n'+
 ...
 clinical:{t:'CLASS B · CLINICAL · care.twc.health and app clinical screens\n'+
  '='.repeat(58)+'\n\n'+
  'BROWSER PIXEL      none\n'+
  'SDK                none\n'+
...
function bdSeg(pfx,per){
  return {s:+$('bd-'+pfx+per+'s').value||0,o:+$('bd-'+pfx+per+'o').value||0,r:+$('bd-'+pfx+per+'r').value||0};
}
function calcBlend(){
  var P0=bdSeg('p','0'),R0=bdSeg('r','0'),E0=bdSeg('e','0');
  var P1=bdSeg('p','1'),R1=bdSeg('r','1'),E1=bdSeg('e','1');
  var paid0=[P0,R0], paid1=[P1,R1];
  var spend0=P0.s+R0.s, spend1=P1.s+R1.s;
  var ord0=P0.o+R0.o+E0.o, ord1=P1.o+R1.o+E1.o;
  var new0=P0.o+R0.o, new1=P1.o+R1.o;
  var cpa0=ord0?spend0/ord0:0, cpa1=ord1?spend1/ord1:0;
  var nc0=new0?spend0/new0:0, nc1=new1?spend1/new1:0;
  ...
  var eff=0,mix=0;
  for(var i=0;i<2;i++){ eff+=w0[i]*(c1[i]-c0[i]); mix+=c0[i]*(w1[i]-w0[i]); }
  var total=nc1-nc0;
  var inter=total-eff-mix;
  var rows=[
   ['Efficiency',eff,'Each segment\u2019s own cost per new customer changed. This is genuine performance movement.'],
   ['Mix',mix,'The share of new customers coming from each segment changed. Cost per segment may not have moved at all.'],
   ['Interaction',inter,'The residual where both moved together. Small residuals are normal; a large one means both effects are substantial.']];
  $('bd-effects').innerHTML=rows.map(function(r){
    var sign=r[1]>=0?'+':'';
    var col=Math.abs(r[1])<1?'inherit':(r[1]<0?'var(--pass)':'var(--fail)');
    return '<tr><td><b>'+r[0]+'</b></td><td class="num" style="color:'+col+'">'+sign+'$'+r[1].toFixed(2)+'</td>'+
      '<td style="font-size:12.5px;color:var(--ink-3)">'+r[2]+'</td></tr>';
  }).join('');

  var pShare0=w0[0]*100, pShare1=w1[0]*100;
  var rd=$('bd-read'); rd.className='readout'; var cls,label,val,body;
  var mixDominant=Math.abs(mix)>Math.abs(eff)*1.4;
  if(d<0&&dn>0){
    cls='is-fail';label='Blended improving, acquisition worsening';val='CANNIBALISATION';
    body='Blended CPA fell '+Math.abs(d).toFixed(1)+' percent while cost per new customer rose '+dn.toFixed(1)+
      ' percent. Retargeting share moved from '+(100-pShare0).toFixed(0)+' to '+(100-pShare1).toFixed(0)+
    ...
    cls='is-warn';label='The mix moved, not the performance';val='MIX';
    body='Of the $'+Math.abs(total).toFixed(2)+' change in cost per new customer, $'+Math.abs(mix).toFixed(2)+
      ' came from the mix shifting and only $'+Math.abs(eff).toFixed(2)+' from efficiency actually changing. Nothing improved. The cheaper segment simply grew as a share. Run the fixed-mix comparison before reporting this as a gain, because a mix that can shift one way will shift back.';
  } else if(mixDominant&&d>0){
    cls='is-warn';label='The mix moved against you';val='MIX';
    body='Cost per new customer rose mainly because the mix shifted toward the more expensive segment, not because any segment got worse. Prospecting share moved from '+pShare0.toFixed(0)+' to '+pShare1.toFixed(0)+
    ...
    cls='is-pass';label='Genuine efficiency gain';val='REAL';
    body='Cost per new customer fell and the efficiency effect of $'+Math.abs(eff).toFixed(2)+
    ...
    cls='is-fail';label='Genuine deterioration';val='REAL';
    body='Cost per new customer rose and the efficiency effect of $'+eff.toFixed(2)+
  ...
  $('bd-label').textContent=label;$('bd-val').textContent=val;$('bd-body').textContent=body;
}
['p','r','e'].forEach(function(pf){['0','1'].forEach(function(pe){['s','o','r'].forEach(function(f){
  var id='bd-'+pf+pe+f; if($(id))$(id).addEventListener('input',calcBlend);
})})});

function calcAcqBlend(){
  var rev=+$('ba-rev').value||0, ret=+$('ba-ret').value||0, sub=+$('ba-sub').value||0,
      sp=+$('ba-sp').value||1, nw=+$('ba-new').value||1, mo=+$('ba-mo').value||1;
  var acq=Math.max(0,rev-ret-sub);
  var broas=rev/sp, aroas=acq/sp, mer=rev/sp, amer=acq/sp, ncac=sp/nw;
  ...
  var n=$('ba-note');
  if(gap<15){n.className='note good';n.textContent='Blended and acquisition ROAS differ by '+gap.toFixed(0)+' percent, because returning and renewal revenue is still only '+baseShare.toFixed(0)+' percent of the total. At month '+mo+' that is expected. The gap will widen every month from here whether acquisition improves or not, so establish the acquisition figure as the reported number now rather than after the divergence becomes awkward to explain.';}
  else if(gap<45){n.className='note warn';n.textContent='Returning and renewal revenue is '+baseShare.toFixed(0)+' percent of the total, making blended ROAS '+gap.toFixed(0)+' percent higher than acquisition ROAS. Budget decisions made on the blended figure are being made on revenue the ads did not generate this period. Report acquisition ROAS as the primary number and blended as context.';}
  else{n.className='note bad';n.textContent='At '+baseShare.toFixed(0)+' percent of revenue from the existing base, blended ROAS of '+broas.toFixed(2)+' overstates acquisition ROAS of '+aroas.toFixed(2)+' by '+gap.toFixed(0)+' percent. The blended figure is now mostly a measure of how large the customer base is rather than how well acquisition works, and it will keep rising even if acquisition stops entirely. Any scaling decision made on it is being made on the wrong number.';}
  var rows='';
  for(var m=mo;m<=mo+9;m+=3){
    var growth=Math.min(0.72,baseShare/100*Math.pow(1.13,m-mo));
    var proj=aroas/(1-growth);
    rows+='<tr'+(m===mo?' style="background:#e8eff7"':'')+'><td class="num">'+m+'</td><td class="num">'+(growth*100).toFixed(0)+'%</td>'+
      '<td class="num">'+proj.toFixed(2)+'</td><td style="font-size:12.5px;color:var(--ink-3)">'+
      (m===mo?'Today':'Reads as a '+((proj/broas-1)*100).toFixed(0)+' percent improvement, with acquisition unchanged')+'</td></tr>';
  }
  $('ba-proj').innerHTML=rows;
}
['ba-rev','ba-ret','ba-sub','ba-sp','ba-new','ba-mo'].forEach(function(id){$(id).addEventListener('input',calcAcqBlend)});


/* ================= MAKE SCENARIOS ================= */
var MK=[
 {k:'health',g:'monitor',n:'Daily account health sweep',rw:'read',eff:'Low',pri:1,
  ...
  bp:'TRIGGER   Schedule · 07:00 daily, account timezone\n\n'+
 ...
 {k:'pace',g:'monitor',n:'Pacing monitor',rw:'read',eff:'Low',pri:4,
  ...
  bp:'TRIGGER   Schedule · 08:00 daily\n\n'+
 ...
 {k:'learn',g:'monitor',n:'Learning phase watcher',rw:'read',eff:'Low',pri:6,
  ...
  bp:'TRIGGER   Schedule · Monday 09:00\n\n'+
 ...
 {k:'recon',g:'monitor',n:'Reconciliation monitor',rw:'read',eff:'Medium',pri:5,
  ...
  bp:'TRIGGER   Schedule · 09:00 daily, for yesterday\n\n'+
 ...
 {k:'freq',g:'monitor',n:'Frequency and saturation alert',rw:'read',eff:'Low',pri:8,
  ...
  bp:'TRIGGER   Schedule · daily 09:00\n\n'+
     '1  HTTP · GET act_{id}/insights\n'+
     '     level        adset\n'+
     '     date_preset  last_7d\n'+
 ...
 {k:'launch',g:'creative',n:'Batch launch pipeline',rw:'write',eff:'High',pri:9,
  ...
  bp:'TRIGGER   Google Sheets · watch rows, Creative Ledger\n'+
 ...
 {k:'read',g:'creative',n:'Day 14 batch reader',rw:'read',eff:'Medium',pri:3,
  ...
  bp:'TRIGGER   Schedule · every 14 days, aligned to the batch cycle\n\n'+
 ...
 {k:'starve',g:'creative',n:'New creative delivery watchdog',rw:'read',eff:'Medium',pri:7,
  ...
  bp:'TRIGGER   Schedule · daily, for ads created in the last 7 days\n\n'+
 ...
 {k:'ledger',g:'creative',n:'Ledger writer on retirement',rw:'read',eff:'Low',pri:10,
 ...
 {k:'marginal',g:'scaling',n:'Marginal CPA tracker',rw:'read',eff:'Medium',pri:2,
  ...
  bp:'TRIGGER   Webhook from the budget change logger, or schedule\n\n'+
 ...
 {k:'budgetlog',g:'scaling',n:'Budget change logger',rw:'read',eff:'Low',pri:11,
  ...
  bp:'TRIGGER   Schedule · daily 06:00\n\n'+
 ...
 {k:'gate',g:'scaling',n:'Expansion gate checker',rw:'read',eff:'Medium',pri:12,
 ...
 {k:'gated',g:'risk',n:'Gated spend guard',rw:'read',eff:'Low',pri:1,
 ...
 {k:'dest',g:'risk',n:'Destination monitor',rw:'read',eff:'Low',pri:13,
  ...
  bp:'TRIGGER   Schedule · every 6 hours\n\n'+
 ...
 {k:'digest',g:'risk',n:'Weekly compliance digest',rw:'read',eff:'Low',pri:14,
  ...
  bp:'TRIGGER   Schedule · Monday 08:00\n\n'+
 ...
 {k:'rxcapi',g:'data',n:'Rx conversion relay',rw:'write',eff:'High',pri:1,
  ...
  bp:'TRIGGER   Shopify · Watch Order Updates\n'+
 ...
 {k:'renew',g:'data',n:'Renewal separator',rw:'write',eff:'Medium',pri:2,
  ...
  bp:'TRIGGER   Shopify · Watch Orders\n'+
 ...
 {k:'survey',g:'data',n:'Post-purchase survey collector',rw:'read',eff:'Low',pri:6,
  ...
  bp:'TRIGGER   Webhook from the thank-you page survey\n\n'+
 ...
 {k:'snapshot',g:'data',n:'Daily metrics snapshot',rw:'read',eff:'Medium',pri:5,
  ...
  bp:'TRIGGER   Schedule · 05:00 daily, for yesterday\n\n'+
 ...
 {k:'custlist',g:'audience',n:'Customer list refresh',rw:'write',eff:'Medium',pri:3,
  ...
  bp:'TRIGGER   Schedule · Monday 04:00\n\n'+
 ...
 {k:'suppress',g:'audience',n:'Suppression sync',rw:'write',eff:'Low',pri:4,
  ...
  bp:'TRIGGER   Schedule · daily 04:30\n\n'+
 ...
 {k:'seed',g:'audience',n:'Lookalike seed refresh',rw:'write',eff:'Medium',pri:11,
  ...
  bp:'TRIGGER   Schedule · quarterly, first Monday\n\n'+
 ...
 {k:'lapse',g:'audience',n:'Lapsed and reactivation segments',rw:'write',eff:'Low',pri:12,
 ...
 {k:'apptag',g:'audience',n:'App user tag writeback',rw:'write',eff:'Medium',pri:13,
  ...
  bp:'TRIGGER   Webhook from Firebase Cloud Function\n'+
 ...
 {k:'fatigue',g:'creative',n:'Fatigue early warning',rw:'read',eff:'Medium',pri:7,
 ...
 {k:'brief',g:'creative',n:'Brief generator on iterate',rw:'read',eff:'Medium',pri:14,
  ...
  bp:'TRIGGER   Google Sheets · watch Creative Ledger\n'+
 ...
 {k:'intake',g:'creative',n:'Asset intake validator',rw:'read',eff:'Low',pri:15,
  ...
  bp:'TRIGGER   Google Drive · watch folder\n\n'+
     '1  Regex on filename\n'+
 ...
 {k:'review',g:'creative',n:'Compliance review SLA tracker',rw:'read',eff:'Low',pri:16,
 ...
 {k:'concentration',g:'scaling',n:'Delivery concentration monitor',rw:'read',eff:'Low',pri:8,
 ...
 {k:'supply',g:'scaling',n:'Creative supply gap alert',rw:'read',eff:'Medium',pri:9,
 ...
 {k:'rollback',g:'scaling',n:'Daily state snapshot for rollback',rw:'read',eff:'Low',pri:17,
  ...
  bp:'TRIGGER   Schedule · 05:00 daily\n\n'+
 ...
 {k:'prescan',g:'risk',n:'Ad copy pre-scan',rw:'read',eff:'Medium',pri:10,
  ...
  bp:'TRIGGER   Webhook from the brief form, or Sheets watch\n\n'+
 ...
 {k:'affiliate',g:'risk',n:'Affiliate destination sweep',rw:'read',eff:'Medium',pri:18,
  ...
  bp:'TRIGGER   Schedule · daily 07:00\n\n'+
 ...
 {k:'appeal',g:'risk',n:'Appeal tracker',rw:'read',eff:'Low',pri:19,
  ...
  bp:'TRIGGER   Webhook or manual entry when an appeal is filed\n\n'+
     '1  Google Sheets · append\n'+
 ...
 {k:'p1',g:'risk',n:'P1 incident runbook',rw:'write',eff:'Medium',pri:5,
  ...
  bp:'TRIGGER   Schedule · every 15 minutes\n'+
 ...
 {k:'refund',g:'data',n:'Refund and chargeback tracker',rw:'read',eff:'Low',pri:20,
  ...
  bp:'TRIGGER   Shopify · Watch Refunds\n'+
 ...
 {k:'dunning',g:'data',n:'Dunning trigger',rw:'write',eff:'Medium',pri:6,
 ...
 {k:'weekly',g:'report',n:'Weekly report assembler',rw:'read',eff:'High',pri:15,
  ...
  bp:'TRIGGER   Schedule · Monday 06:00\n\n'+
 ...
 {k:'anomaly',g:'report',n:'Statistical anomaly detector',rw:'read',eff:'Medium',pri:9,
 ...
 {k:'cohort',g:'report',n:'Monthly cohort snapshot',rw:'read',eff:'Medium',pri:16,
  ...
  bp:'TRIGGER   Schedule · first of the month\n\n'+
 ...
 {k:'oneThing',g:'report',n:'Daily one-thing digest',rw:'read',eff:'Low',pri:21,
  ...
  bp:'TRIGGER   Schedule · 07:30 daily\n\n'+
...
var mkKey='monitor';
function renderMake(){
  var set=MK.filter(function(m){return m.g===mkKey});
  $('mk-list').innerHTML=set.map(function(m){
    var cls=m.rw==='write'?'f-med':'f-ok';
    return '<div class="flag '+cls+'"><b>'+m.n+
      ' <span style="font-weight:400;font-size:12px;color:var(--ink-3)">· '+
      (m.rw==='write'?'writes to Meta':'read only')+' · '+m.eff+' effort</span></b>'+m.d+'</div>';
  }).join('');
}
$('mk-tabs').addEventListener('click',function(e){
  var b=e.target.closest('.tab'); if(!b)return;
  Array.prototype.forEach.call($('mk-tabs').children,function(x){x.classList.remove('on')});
  b.classList.add('on'); mkKey=b.dataset.mk; renderMake();
});
(function(){
  var groups={monitor:'Monitor',creative:'Creative ops',audience:'Audiences',scaling:'Scaling',risk:'Compliance',data:'Data',report:'Reporting'};
  var h='';
  Object.keys(groups).forEach(function(g){
    h+='<optgroup label="'+groups[g]+'">'+MK.filter(function(m){return m.g===g}).map(function(m){
      return '<option value="'+m.k+'">'+m.n+'</option>';
    }).join('')+'</optgroup>';
  });
  $('mk-sel').innerHTML=h;
})();
function renderMakeBp(){
  var m=MK.filter(function(x){return x.k===$('mk-sel').value})[0]||MK[0];
  $('mk-out').textContent=m.n.toUpperCase()+'\n'+'='.repeat(Math.min(58,m.n.length+8))+'\n\n'+m.bp;
  $('mk-meta').textContent=(m.rw==='write'?'writes to Meta':'read only')+' · '+m.eff+' effort';
  $('mk-note').className='note'+(m.rw==='write'?' warn':'');
  $('mk-note').textContent=m.note;
}
$('mk-sel').addEventListener('change',renderMakeBp);
(function(){
  var ranked=MK.slice().sort(function(a,b){return a.pri-b.pri}).slice(0,12);
  $('mk-order').innerHTML=ranked.map(function(m,i){
    return '<tr><td class="num">'+(i+1)+'</td><td><b>'+m.n+'</b></td>'+
      '<td>'+(m.rw==='write'?'<span class="pill p-iterate">Writes</span>':'<span class="pill p-scale">Read</span>')+'</td>'+
      '<td>'+m.eff+'</td><td style="font-size:12.5px;color:var(--ink-3)">'+m.d.split('.')[0]+'.</td></tr>';
  }).join('');
})();
