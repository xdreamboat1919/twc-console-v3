...
function pad(n){return String(Math.max(1,parseInt(n)||1)).padStart(2,'0')}
function esc(s){return s.replace(/[&<>]/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;'}[c]})}

/* ================= NAVIGATION ================= */
var SECMETA={};
NAV.forEach(function(g){g[2].forEach(function(it){SECMETA[it[0]]={name:it[1],group:g[0],icon:it[2]}})});
function navHTML(){
  var n=0,h='';
...
function setActive(v){
  Array.prototype.forEach.call(document.querySelectorAll('#nav a,#nav2 a'),function(x){
  ...
  Array.prototype.forEach.call(document.querySelectorAll('#nav details,#nav2 details'),function(group){
  ...
  if(v==='build')setTimeout(function(){svFit();cvFit()},80);
...
/* ================= COMMAND PALETTE ================= */
var CMD=[];
NAV.forEach(function(g){g[2].forEach(function(it){
  CMD.push({t:it[1],g:g[0],i:it[2],act:function(v){return function(){setActive(v)}}(it[0])});
})});
var cmdSel=0,cmdCur=[];
function cmdRender(q){
  q=(q||'').toLowerCase().trim();
  cmdCur=CMD.concat(Object.keys(MAPS).map(function(k){
    return {t:MAPS[k].t,g:'Map',i:'bi-bounding-box',act:function(){setActive('maps');setTimeout(function(){selectMap(k)},60)}};
  })).concat(SCEN.map(function(sc){
    return {t:sc.n,g:'Scenario',i:'bi-signpost-split',act:function(){setActive('scen');$('sc-sel').value=sc.k;renderScen()}};
  })).concat((typeof DIAGNOSES==='undefined'?[]:DIAGNOSES).map(function(diag){
    return {t:diag.n,g:'Diagnosis · '+diag.cat,i:'bi-search-heart',act:function(){setActive('diag');selectDiagnosis(diag.k)}};
  }));
  if(q)cmdCur=cmdCur.filter(function(c){return (c.t+' '+c.g).toLowerCase().indexOf(q)>-1});
  cmdSel=0;
  $('cmdList').innerHTML=cmdCur.length?cmdCur.slice(0,40).map(function(c,i){
    return '<div class="cmditem'+(i===0?' sel':'')+'" data-i="'+i+'"><i class="bi '+c.i+'"></i>'+c.t+'<span class="g">'+c.g+'</span></div>';
  }).join(''):'<div class="cmdempty">Nothing matches that.</div>';
}
function cmdMove(d){
  var items=$('cmdList').querySelectorAll('.cmditem'); if(!items.length)return;
  cmdSel=Math.max(0,Math.min(items.length-1,cmdSel+d));
  Array.prototype.forEach.call(items,function(x,i){x.classList.toggle('sel',i===cmdSel)});
...
document.addEventListener('DOMContentLoaded',function(){
  var box=$('cmdBox');
  if(box){
    box.addEventListener('shown.bs.modal',function(){$('cmdInput').value='';cmdRender('');$('cmdInput').focus()});
  }
  ...
  $('save2').addEventListener('click',function(){$('save').click()});
  $('load2').addEventListener('click',function(){$('load').click()});
...
document.addEventListener('click',function(e){
  ...
  navigator.clipboard.writeText($(b.dataset.c).textContent).then(function(){
